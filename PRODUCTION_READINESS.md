...
covered by 234 host-run unit tests, and hardened against the receive path being
...
| V70 | **High** → fixed | `IoHomeControl` 2W pairing, `AuthenticationManager`, `check_no_key_material.py` tests | A sixth review round surfaced one **High** auth-bypass and one medium test-quality gap; both fixed. **Unsolicited key adoption (High):** a fresh 2W node in `accept_pairing` mode adopted an *attacker-chosen* system key from a single unsolicited 0x32 (`CMD_KEY_TRANSFER`), because the follower branch unmasked the key without ever having issued a 0x3C challenge. On a fresh node `pairing_peer_` and `current_challenge_` are still all-zero, so a 0x32 from source `00:00:00` satisfied the `from_peer` check and unmasked against the zero challenge - no 0x31/0x3C handshake needed - letting a passer-by set the installation key. Three defenses close it: the follower now requires an active challenge (`has_active_challenge`) before recovering a pushed key; `recover_2w_key()` refuses to unmask against an all-zero `current_challenge_` (a nonce the CSPRNG never emits); and `handle_pairing_frame()` drops any pairing frame whose source is `00:00:00` so it can never match a zeroed `pairing_peer_`. A regression test forges an unsolicited CRC-valid 0x32 from the group address and from a stranger and asserts no 0x33 ack, no callback, and that the node still authenticates under its original key - mutation-verified (removing all three defenses fails it). **Circular CRC in scanner test (Medium test-gap):** `test_check_no_key_material.py` built its fixtures with the scanner's own `crc16_kermit`, so a bug in that function would move fixture and validator together and every assertion would still pass against a broken CRC; added an offline-computed CRC-16/KERMIT known-answer vector (cross-checked with an independent non-reflected 0x1021 implementation) that trips if `crc16_kermit` ever diverges from the true polynomial. |
| V71 | Medium | docs, `IoHomeControl`, tests | A seventh review round surfaced three confirmed findings, all self-inflicted by round 6's own fix; no new code vulnerability. **Doc understated the in-window attack (Medium, two lenses):** V70's new SECURITY-MODEL.md paragraph called the residual during an open pairing window a garbage-key DoS - "the attacker cannot choose the resulting key, only corrupt it." That is wrong. Every input to the 2W key mask `AES-128(TRANSFER_KEY, IV)` is attacker-known (the transfer key is the public constant, the request byte is a protocol constant, and the challenge is broadcast in the clear in the node's own 0x3C), so an attacker active in the window sends its own 0x31, reads the challenge, and pushes a 0x32 whose payload is `K_chosen XOR mask` - the node adopts a key the attacker *picked and knows*, gaining authenticated control until re-pair. Corrected to describe it as chosen-key injection (the same disclosure as a passive listener recovering the operator's key, only active), while keeping the correct conclusion that it is inherent to plaintext key transfer and bounded to the operator-opened window. **has_active_challenge gate untested in its blocking role (Medium test-gap):** V70's regression test forges 0x32s that are stopped by the all-zero-src drop and the `from_peer` check *before* reaching the `has_active_challenge` gate, so deleting that gate alone left the whole suite green. Since `recover_2w_key()` reads `current_challenge_` directly and never re-checks expiry, the gate is the only thing that rejects a stale-but-still-non-zero challenge - a real exposure in the window after a challenge times out but before anything polls it. Added a test that answers a peer's 0x31, advances past the 5 s challenge timeout on the pairing clock, then delivers a spoofed 0x32; mutation-verified (removing *only* the gate fails the new test while the unsolicited test still passes). Routed the follower's challenge lifecycle through the existing `pairing_now_ms()` seam to make the timeout testable (identical to `NOW_MS()` in production). One finder claim - that the adoption is not peer-bound and needs no handshake - was refuted (it does require completing a 0x31/0x3C exchange). |
| V72 | Low | `iohome_aes_soft.cpp`, `IoHomeControl.cpp`, tests | An eighth review round surfaced two confirmed Low findings, both in long-standing code rather than the recent fixes; no new attack. **Software-AES key scrub elided (Low, defense-in-depth):** `soft_aes::encrypt_block()`/`decrypt_block()` ended with a plain `memset` of the 176-byte key schedule and the state; both locals die on return, so at `-O2` the compiler removes the memset as a dead store, leaving the raw AES key and schedule on the stack. This is the one crypto site that used plain `memset` instead of the volatile-pointer `crypto::secure_zero` the rest of the layer relies on. Only affects host / software-AES builds (production ESP32 uses mbedTLS), hence Low. Switched both scrubs to `secure_zero`; verified at the object-code level that the calls now survive `-O2` (four `secure_zero` relocations emitted before each `ret`, where the disassembly previously went straight from the output store to the epilogue). **Eager per-boot rolling-code reservation (Low, self-inflicted lockout):** `begin()` advanced and persisted the transmit rolling code by a full reserve block (default 64) on *every* boot, even when the previous run transmitted nothing. A run of no-op reboots (brownouts, a re-init loop) therefore skipped the counter forward by 64 each time; after ~16 the counter passes the receiver's bounded replay window (1000) and every subsequent command is rejected as out-of-window, with no attacker involved and no self-heal. Deferred the reservation: `begin()` now only restores the stored counter, and the first `consume_rolling_code()` reserves a block and persists it *before* handing out a code - so the no-reuse guarantee holds while a transmit-nothing boot costs zero codes. Added `test_rolling_code_noop_reboots_burn_nothing` (20 no-op reboots must not touch flash; the first real transmit still persists ahead of the code) and updated the two batching/resume tests to the deferred cadence; all mutation-verified against the eager code. |
| V73 | Low | `esphome/components/iown_homecontrol/iown_homecontrol.cpp` | A ninth review round confirmed a single finding: V72's rolling-code fix landed in the library (`src/IoHomeControl.cpp`) but not in the ESPHome component, which keeps its own parallel copy of the counter logic (`iown_homecontrol.cpp` is component glue, not one of the files `sync_esphome_protocol.py` mirrors). `restore_rolling_code_()` still reserved and persisted a full block on every boot, so the exact V72 lockout (a run of no-op reboots skipping the transmit counter past the actuator's window) applied to the ESPHome hub. Ported the deferred reservation: `restore_rolling_code_()` now only restores the stored counter, and the unchanged `consume_rolling_code_()` reserves + persists the first block before handing out a code. Verified by `esphome compile`; the host suite compiles no ESPHome sources, so there is no host-level test for this path (same structural limitation noted for the ESPHome cover gating). The round's other four lenses - NVS/rolling-code store internals, beacon/discovery parsing, the Velux helper builders, and PHY framing / ChannelHopper / the legacy layer - returned clean. |
| V74 | Medium | `.github/workflows/`, docs | A tenth review round swept the last un-lensed surface (CI/tooling, cross-cutting seams, whole-tree docs, the untouched `esp32_api_spi`/`board_pins`/`main`/mocks); the protocol, crypto, controller and ESPHome code lenses all returned clean, and it surfaced three non-code findings. **Board-build CI unconditionally red (Medium):** the `boards` job in `platformio.yml` enumerates every env and runs `pio run -e <env>`, skipping only the exact names `native|debug`. The board-less `native_nosan` env (added later, `extends = env:native`) was not skipped, so `pio run -e native_nosan` tried to build a native env with no `main()` and failed under `set -euo pipefail`, reddening the job on every matching push/PR and masking real board regressions. Widened the skip to `native*|debug` (verified by simulating the enumeration: `native`, `native_nosan`, `debug` skipped; all ten real boards kept). **Scripts guard never triggered (Low):** `tests.yml`'s `scripts` job byte-compiles `scripts/` and runs the frame-parser self-test, but the workflow's path filters omitted `scripts/**`, so a scripts-only change ran neither guard - the same "path-filtered workflow misses an unwatched dir" bug class `no-key-material.yml` exists to avoid. Added `scripts/**` to both the push and pull_request path lists. **Doc offset wrong (Low):** the HARDWARE-BRINGUP FP1 tilt experiment told a tester to read "offset 5 of the payload", but Functional Parameter 1 is at Execute-payload offset 4 (`EXECUTE_OFFSET_FP1`; offset 5 is FP2) - following it would have read the wrong byte. Corrected to offset 4 with the layout spelled out. |
| V75 | Medium | `.github/workflows/` | An eleventh review round re-verified round 10's CI fixes and re-confirmed the code; the code re-confirmation and doc-count lenses returned clean, leaving two more non-code findings - both the same class as round 10's. **`include/**` missing from `tests.yml` triggers (Medium):** exactly the gap round 10 fixed for `scripts/**`, left for `include/`. `tests.yml` carries guards that exist only to protect `include/` - the "Compile the C headers as C" step (`gcc -I include` on a probe that catches an `enum class` inside an `extern "C"` block, which `include/iown_node_types.h` still has) and the `_Static_assert` frame-length-macro probe against `include/iown_frame.h` - yet `include/**` was in neither path filter. An include-only change (e.g. flipping `IOWN_FRAME_SIZE_BIAS`, which `pio run`/CodeQL compile green) would skip the one workflow written to catch it. Added `include/**` to both path lists; `tests.yml` now triggers on every directory it guards. **Stale `arduino-lint.yml` header comment (Low):** the header argued for `library-manager: false` because "three tracked .exe files ... block addition to the index" and pointed at a nonexistent `K10`. All false now: the actual config is `library-manager: submit`, `git ls-files` shows zero tracked `.exe` files (removed in V47), and there is no `K10` (V68 already scrubbed one phantom K10 but missed this copy). Rewrote the comment to state that submit mode passes clean since V47 removed the executables, and repointed the reference at V47. |
| V76 | Low | `.github/workflows/codeql.yml`, `keywords.txt` | A twelfth review round swept every workflow, manifest and doc cross-reference in one pass and re-confirmed the code a third time; the code and doc-crossref lenses returned clean, leaving two more Low non-code nits of the same staleness class. **Stale CodeQL header (Low):** `codeql.yml`'s header told the reader to "modify the if step" and edit a "commented block [that] configures Python and PlatformIO" - neither exists; the file builds via an unconditional "Build for analysis" g++ step with `build-mode: manual` already set (carried over verbatim from GitHub's CodeQL starter template). Rewrote the header to describe the actual build step. **Stale keyword (Low):** `keywords.txt` listed `DeviceType`, an enum renamed to `NodeType` back at V59 (`git grep` finds `DeviceType` in no `src/`/`include/` source), while the current public `NodeType` enum (`iohome_constants.h`) was unlisted - only Arduino IDE syntax coloring is affected. Replaced the entry with `NodeType`. |
...
| Area | Status | Notes |
|------|--------|-------|
| Unit tests | ✅ 234 tests | 9 suites, ASan + UBSan by default |
...
K2, K3, K4, K5, K6, K7 and K9 are closed - see V47 to V62 above. K2 was
